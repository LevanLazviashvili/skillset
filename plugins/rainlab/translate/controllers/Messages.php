...
class Messages extends Controller
{
    ...
    public function onClearCache()
    {
        ...
        Message::all()->map(function ($message) use (&$Result, $columns) {
            foreach ($columns AS $column) {
                $Result[$column][$message->code] = Arr::get($message->message_data, $column, '');
            }
        })->toArray();
        foreach ($Result AS $key => $text) {
            File::put(storage_path().'/app/langs/'.$key.'.json', json_encode($text));
        }
        $Versions = json_decode(File::get(storage_path().'/app/versions.json', 0), 1);
        $Versions['langs'] += 1;
        File::put(storage_path().'/app/versions.json', json_encode($Versions));
